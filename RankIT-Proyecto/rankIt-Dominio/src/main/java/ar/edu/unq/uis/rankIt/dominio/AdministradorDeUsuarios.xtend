...
class AdministradorDeUsuarios {
	...
	def Usuario buscarUsuarioPorNombre(String nombreDeUsuario) {
		return this.usuarios.findFirst[ each | each.nombre.equals(nombreDeUsuario) ]
	}
	
	def Usuario buscarUsuarioPorId(Integer idUsuario) {
		return this.usuarios.findFirst[ each | each.id.equals(idUsuario) ]
	}
